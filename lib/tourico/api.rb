  ...
  class Api
    ...
    def services (action, args)
      ...
      if reservations_services.include?(action)
        return HTTPService.make_request_reservation_service(action, args, settings)
      end

      if hotel_services.include?(action)
        return HTTPService.make_request(action, args, settings)
      end

      if destination_services.include?(action)
        return HTTPService.make_request_destination_service(action, args, settings)
      end
    end
  end
end